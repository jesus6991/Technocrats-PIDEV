...
namespace TestIdentity
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                //var user = new Client { UserName = model.Email, Email = model.Email,Password=model.Password };
                //var result = await UserManager.CreateAsync(user, model.Password);
                //if (result.Succeeded)
                //{
                    if (model.Roles == "Client")
                    {
                        var user = new Client { UserName = model.Email, Email = model.Email, Password = model.Password, categorie_c=ClientCategory.Private, type_c=ClientType.NewClient};
                        var result = await UserManager.CreateAsync(user, model.Password);
                    if (result.Succeeded)
                    {
                        UserManager.AddToRole(user.Id, "Client");
                        //Client client = new Client();
                        //client.adresse = model.Email;
                        //ClientService cs = new ClientService();
                        //cs.Add(client);
                        //cs.Commit();
                        return RedirectToAction("HomeClient", "Home");
                    ...
                    {
                    var user = new Ressource { UserName = model.Email, Email = model.Email, Password = model.Password,Avaibility= AvailibilityType.Available };
                    var result = await UserManager.CreateAsync(user, model.Password);
                    if (result.Succeeded)
                    {
                    ...
                    {
                    var user = new Applicant { UserName = model.Email, Email = model.Email, Password = model.Password, applicant_state= ApplicantState.Refused};
                    var result = await UserManager.CreateAsync(user, model.Password);
                    if (result.Succeeded)
                    {
